...
Custom iMessage channel for Claude Code, forked from the [official iMessage plugin](https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins/imessage). Reads `~/Library/Messages/chat.db` directly for history, search, and new-message detection; sends via AppleScript to Messages.app. No external server, no background process to keep alive.
...
```
/plugin install zeb-message@ZebLawrence-agent-resources
```
...
```sh
claude --dangerously-load-development-channels --channels plugin:zeb-message@ZebLawrence-agent-resources
```

Check that `/zeb-message:configure` tab-completes.
...
| `IMESSAGE_STATE_DIR` | `~/.claude/channels/zeb-message` | Override where `access.json` and pairing state live. |
