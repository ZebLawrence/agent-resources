...
Texting yourself always works. **Self-chat bypasses the gate** with no setup: the server learns your own addresses at boot and lets them through unconditionally. For other senders, the default policy is **`allowlist`**: nothing passes until you add the handle with `/zeb-message:access allow <address>`.

All state lives in `~/.claude/channels/zeb-message/access.json`. The `/zeb-message:access` skill commands edit this file; the server re-reads it on every inbound message, so changes take effect without a restart. Set `IMESSAGE_ACCESS_MODE=static` to pin config to what was on disk at boot.
...
| Config file | `~/.claude/channels/zeb-message/access.json` |
...
```
/zeb-message:access policy pairing
```
...
```
/zeb-message:access allow +15551234567
/zeb-message:access allow friend@icloud.com
/zeb-message:access remove +15551234567
```
...
```
/zeb-message:access group add "iMessage;+;chat123456789012345678"
```
...
```
/zeb-message:access set mentionPatterns '["^claude\\b", "@assistant"]'
```

Pass `--no-mention` to process every message in the group, or `--allow addr1,addr2` to restrict which members can trigger it.

```
/zeb-message:access group add "iMessage;+;chat123456789012345678" --no-mention
/zeb-message:access group add "iMessage;+;chat123456789012345678" --allow +15551234567,friend@icloud.com
/zeb-message:access group rm "iMessage;+;chat123456789012345678"
```

## Delivery

AppleScript can send messages but cannot tapback, edit, or thread-reply; those require private API. Delivery config is correspondingly limited. Set with `/zeb-message:access set <key> <value>`.
...
| Command | Effect |
| --- | --- |
| `/zeb-message:access` | Print current state: policy, allowlist, pending pairings, enabled groups. |
| `/zeb-message:access pair a4f91c` | Approve a pending code (relevant only under `pairing` policy). |
| `/zeb-message:access deny a4f91c` | Discard a pending code. |
| `/zeb-message:access allow +15551234567` | Add a handle. The primary entry point under the default `allowlist` policy. |
| `/zeb-message:access remove +15551234567` | Remove from the allowlist. |
| `/zeb-message:access policy pairing` | Set `dmPolicy`. Values: `pairing`, `allowlist`, `disabled`. |
| `/zeb-message:access group add "iMessage;+;chat..."` | Enable a group. Quote the GUID. Flags: `--no-mention`, `--allow a,b`. |
| `/zeb-message:access group rm "iMessage;+;chat..."` | Disable a group. |
| `/zeb-message:access set textChunkLimit 5000` | Set a config key: `textChunkLimit`, `chunkMode`, `mentionPatterns`. |

## Config file

`~/.claude/channels/zeb-message/access.json`. Absent file is equivalent to `allowlist` policy with empty lists: only self-chat passes.
